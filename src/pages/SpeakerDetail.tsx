  ...
  if (!speaker) {
    return (
      <div style={{ minHeight: '100vh', background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 16 }}>
        <p style={{ fontSize: 22, fontWeight: 700, color: '#ffffff' }}>Speaker not found</p>
        <Link to="/speakers" style={{ color: 'var(--blue)', fontWeight: 600, fontSize: 15 }}>← Back to Speakers</Link>
      </div>
    )
  }
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            ...
            <div style={{ display: 'flex', gap: 36, alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div style={{
                ...
                {speaker.photo ? (
                  ...
                  <span style={{ fontSize: 48, fontWeight: 700, color: 'var(--blue)' }}>{speaker.initials}</span>
                )}
              </div>
              <div style={{ flex: 1, minWidth: 220 }}>
                ...
                <p style={{ color: 'var(--blue)', fontWeight: 700, fontSize: 16, marginBottom: 6 }}>{speaker.title}</p>
                    ...
                    {speaker.links.map((link) => (
                      <a
                        ...
                        style={{
                          ...
                          background: 'var(--blue-dim)', border: '1px solid rgba(129,140,248,0.25)',
                          padding: '5px 12px', borderRadius: 8, transition: 'opacity 0.2s',
                        }}
                      >
                        {link.type === 'web' && 'Website'}
                        {link.type === 'orcid' && 'ORCID'}
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 32 }}>
              <h2 style={{ fontSize: 19, fontWeight: 700, color: '#ffffff', marginBottom: 16 }}>Biography</h2>
              <p style={{ color: 'rgba(255,255,255,0.7)', lineHeight: 1.8, fontSize: 16 }}>{speaker.bio_en}</p>
            </div>
          </div>
        </div>
      </div>
